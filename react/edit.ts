...
export const CRMContactList = () => {
  ...
  useEffect(() => {
    setGridDataSource(
      new DataSource({
        key: 'id', // Use the unique identifier for your data
        load: () => getContacts(), // Load data from your API or data provider
        update: (key, values) => {
          // Add your update logic here
          return new Promise<void>((resolve, reject) => {
            // Example: Make an API call to update the record
            fetch(`https://example.com/api/contacts/${key}`, {
              method: 'PUT',
              headers: {
              ...
              .then((response) => {
                if (response.ok) {
                  ...
                  reject('Failed to update the record');
                }
              })
  ...
  const handleBatchSave = (changes) => {
    console.log("Batch changes:", changes);
  
    // Group changes by type
    const addedRows = changes.filter((change) => change.type === "insert");
    const updatedRows = changes.filter((change) => change.type === "update");
    const deletedRows = changes.filter((change) => change.type === "remove");
  
    // Example: Send each group of changes to the server
    Promise.all([
      ...
      .then(() => {
        ...
        notify("All changes saved successfully", "success", 2000);
      })
      .catch((error) => {
        console.error("Batch save failed", error);
        notify("Failed to save changes", "error", 2000);
      });
  };

  const handleAddedRows = (addedRows) => {
    if (addedRows.length === 0) return Promise.resolve();
  
    return fetch("https://example.com/api/contacts", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify(addedRows.map((row) => row.data)), // Send only the added data
    });
  };

  const handleUpdatedRows = (updatedRows) => {
    if (updatedRows.length === 0) return Promise.resolve();
  
    return Promise.all(
      updatedRows.map((row) =>
        fetch(`https://example.com/api/contacts/${row.key}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify(row.data), // Send updated fields
        })
      )
    );
  };

  const handleDeletedRows = (deletedRows) => {
    if (deletedRows.length === 0) return Promise.resolve();
    ...
    <div className='view crm-contact-list'>
      <div className='view-wrapper view-wrapper-contact-list list-page'>
        ...
        >
          ...
          {/* Add Batch Editing */}
          <Editing
            mode="batch" // Enables batch editing
          ...
          <Toolbar>
            ...
            {/* Add Save and Cancel Buttons for Batch Editing */}
